...
class Sample
{
    ...
    {
        $baseDir = realpath(ROOT_PATH.'/app');
    ...
    public function write(Spreadsheet $spreadsheet, $filename, array $writers = ['Xlsx', 'Xls']): void
    {
        ...
        foreach ($writers as $writerType) {
            $path = $this->getFilename2($filename, mb_strtolower($writerType));
     ...
     * @return string
     */
    public function getFilename2($filename, $extension = 'xlsx')
    {
        $originalExtension = pathinfo($filename, PATHINFO_EXTENSION);
        $baseDir = realpath(ROOT_PATH.'/app/temp/newExcel');
    ...
    public function log($message): void
    {
        ...
        echo date('H:i:s ',time()+8*3600) . $message . $eol;
    }
     ...
     * @param $query
     */
    public function autoTozh($query){
        $from = "auto";
        $to = "zh";
